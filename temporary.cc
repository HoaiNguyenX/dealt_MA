...
namespace Nonlinear {
  ...
  double Nonlinear2D_Benchmark::compute_residual_for_steplength(double alpha
  ) const {
    ...
    //const unsigned int nvf = GeometryInfo<2>::vertices_per_face;
  ...
  void Nonlinear2D_Benchmark::run()
  {
    ...
    while (tria.n_levels() < this -> ref + 1)
    {
      ...
      do {
        ...
        std::cout << "Residual of N_It after assemble" << newton_iteration+1 
            << ":   " << std::fixed << std::setprecision(8) << system_rhs.l2_norm();

        solve_system();
        current_residuals = system_rhs;
        print_numerical_solution(
          "newton_it" + std::to_string(newton_iteration+1) + "_2"
        );
        // compute max value of current_solution
        double max_value = 0.;
        for (unsigned int i = 0; i < current_solution.size(); i++)
          if (current_solution[i] > max_value)
            max_value = current_solution[i];

        std::cout << "    Max value of current_solution: " 
                  << std::fixed << std::setprecision(8) << max_value << std::endl;

        current_residual = system_rhs.l2_norm();
        std::cout << "Residual of N_It after solve" << newton_iteration+1 
