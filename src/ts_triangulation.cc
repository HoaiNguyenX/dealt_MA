  ...
  */
    ...
    for (const auto& cell : this -> active_cell_iterators()) 
    {
      ...
      for (const unsigned int q : ts_values.quadrature_point_indices())
      {
        ...
        for (const unsigned int i : ts_values.dof_indices())
        {
          ...
          // root and squares cancel out -> res_cell = cell_width^1 * norm^1
          cell_residual += (ts_values.shape_grad(i, q)        // \nabla \phi_i
                               * coeff                        // * a_n
                               * sol_grad[q])                 // * \nabla u_n
                               * ts_values.JxW(q);            // * dx
    ...
    ) const 
  {
    ...
    for (const auto& [_, e] : res)
      residuals(i++) = e; 
  } // minimal_residual_error_estimate() [2/2]
    ...
    for (const auto& cell : coarse_neighborhood) {
      ...
      for (const auto& face : inner_faces){
        ...
        for (auto& selected : selected_face){
          ...
          // Generate the new solutions by linear combination of old solutions with same factor
          std::vector<double> new_solution(p[dr] + 2);
          new_solution[0]   = selected[0] -> get_solution();
          new_solution[p[dr] + 1] = selected[p[dr]] -> get_solution();
          for (unsigned int k = 1; k < p[dr] + 1 /* = n_kv - (p[dr] + 2)  - 1*/; k++){
            ...
            new_solution[k] = alpha*selected[k] -> get_solution() 
                              + (1. - alpha)*selected[k-1] -> get_solution(); // a_i * u_i + (1-a_i) * u_(i-1)
          }
          ...
          for (unsigned int k = 0; k < p[dr] + 2 ; k++){
            bool offset = (k > kappa); // TODO: Warum offset?
  ...
  template<int dim, int spacedim>
  void TS_TriangulationBase<dim, spacedim>::transfer_solution(
      Vector<double>& solution
  ) const{
    std::cout << indent(2) << "Transferring solution ... " << std::endl;

    const auto active_splines = this -> get_splines();

    for (unsigned int i = 0; i < this -> n_splines; i++)
      solution[i] = active_splines[i] -> get_solution();
  } // transfer_solution()
